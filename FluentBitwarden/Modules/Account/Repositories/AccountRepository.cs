...
internal sealed class AccountRepository(IConnectionFactory connectionFactory) : IAccountRepository
{
    public async Task<IReadOnlyList<StoredAccount>> GetAccountsAsync(CancellationToken cancellationToken = default)
    {
        ...
        IEnumerable<AccountData> rows = await connection.QueryAsync<AccountData>(command);
        return rows.Select(static row => row.ToStoredAccount()).ToArray();
    }

    public async Task<StoredAccount?> GetByIdAsync(UserId accountId, CancellationToken cancellationToken = default)
    {
        await using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);
        ...
        AccountData? row = await connection.QuerySingleOrDefaultAsync<AccountData?>(command);
        return row?.ToStoredAccount();
    }

    public async Task UpsertAsync(StoredAccount account, CancellationToken cancellationToken = default)
    {
        await using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);
    ...
    public async Task RemoveAsync(UserId accountId, CancellationToken cancellationToken = default)
    {
        await using var connection = await connectionFactory.OpenConnectionAsync(cancellationToken);

...
{
    private static string ReadRequiredDecryptField(ref Utf8JsonReader reader, DecryptedVaultCipherKey decryptKey,
        string propertyName) => ReadDecryptField(ref reader, decryptKey) ??
                                throw new JsonException($"{propertyName} must not be null.");

    private static int ReadRequiredEncryptedInt32(ref Utf8JsonReader reader, DecryptedVaultCipherKey decryptKey)
    ...
    private static bool ReadRequiredEncryptedBoolean(ref Utf8JsonReader reader, DecryptedVaultCipherKey decryptKey)
        => EncryptedJsonValueReader.ReadRequired(ref reader, decryptKey, static (scoped value) =>
        {

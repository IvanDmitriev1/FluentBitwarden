...
internal sealed class SessionIpcHandler(IVaultSession vaultSession)
    : ISessionClient, IIpcRequestsHandler
{
    ...
    {
        vaultSession.TryGetUnlockedSession(out var session);
    ...
    {
        var status = vaultSession.TryGetUnlockedSession(out _)
    ...
    public ValueTask<AccountUnlockOutcome> UnlockAsync(
        ...
        vaultSession.UnlockAsync(request, cancellationToken);
    ...
    public async ValueTask LockAsync(CancellationToken cancellationToken = default) =>
        await vaultSession.LockAsync(cancellationToken);
}

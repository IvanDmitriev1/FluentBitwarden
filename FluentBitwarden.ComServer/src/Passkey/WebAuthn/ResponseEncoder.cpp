...
{
    void EncodeGetAssertion(const PasskeyAssertionResponse& assertion, PWEBAUTHN_PLUGIN_OPERATION_RESPONSE response)
    {
    ...
    void EncodeMakeCredential(const PasskeyMakeCredentialResponse& credential, PWEBAUTHN_PLUGIN_OPERATION_RESPONSE response)
    {
        THROW_HR_IF(E_INVALIDARG, credential.AuthenticatorData.empty());

        WEBAUTHN_CREDENTIAL_ATTESTATION attestation{};
        attestation.dwVersion = WEBAUTHN_CREDENTIAL_ATTESTATION_CURRENT_VERSION;

        attestation.cbAuthenticatorData = static_cast<DWORD>(credential.AuthenticatorData.size());
        attestation.pbAuthenticatorData = const_cast<PBYTE>(credential.AuthenticatorData.data());
        ...
        THROW_HR_IF(
            E_UNEXPECTED,
            encodedSize == 0 ||
            encodedResponse == nullptr);

        response->cbEncodedResponse = encodedSize;
        response->pbEncodedResponse = encodedResponse;
    }
}

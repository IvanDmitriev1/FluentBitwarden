...
namespace BitwardenApi.Vault.Infrastructure;
    ...
    IBitwardenEnvironmentAccessor environmentAccessor) : IVaultApiClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rawValue = await response.Content.ReadFromJsonAsync(BitwardenVaultJsonContext.ConfiguredDefault.Int64, cancellationToken);
        if (rawValue < 0)
    ...
    public async Task<VaultCipherDto> GetCipherAsync(CipherId cipherId, CancellationToken cancellationToken = default)
    {
        ...
        return JsonSerializer.Deserialize(stream, BitwardenVaultJsonContext.Default.VaultCipherDto);
    }

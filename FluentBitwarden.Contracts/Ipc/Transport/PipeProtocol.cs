...
namespace FluentBitwarden.Contracts.Ipc.Transport;
...
internal static class PipeProtocol
{
    ...
    {
        using ArrayPoolBufferWriter<byte> payloadWriter = new();
        MemoryPackSerializer.Serialize(payloadWriter, new Abstractions.IpcOptional<TMessage>(message));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = MemoryPackSerializer.Deserialize<Abstractions.IpcOptional<TResponse>>(bufferOwner.Memory.Span);
        return result.Value;
    }
